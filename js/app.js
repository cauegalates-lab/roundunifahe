const state = {
  ...
  isUpdating: false,
  usarFirebaseTempoReal: false,
  firebaseApp: null,
  firebaseRef: null,
  firebasePrimeiraLeitura: true,
  pendingDashboard: null,
  pendingDashboardOrigem: ''
};
...
function iniciarPainel() {
  ...
  const firebaseAtivo = iniciarFirebaseTempoReal();
  if (!firebaseAtivo) {
  ...
  $('#btnRefresh')?.addEventListener('click', atualizarAgora);

  if (!firebaseAtivo && CONFIG.googleSheets?.atualizarAoFocarPagina !== false) {
...
function atualizarAgora() {
  if (state.usarFirebaseTempoReal) {
    processarDashboardRecebido(state.lastDashboard, false, 'Firebase');
    return;
  }
...
function pausarAtualizacaoDuranteAnimacao(duracaoMs = 4200) {
  ...
  state.resumeRefreshTimer = setTimeout(() => {
    ...
    if (state.pendingDashboard) {
      const dashboardPendente = state.pendingDashboard;
      const origemPendente = state.pendingDashboardOrigem || 'Firebase';
      state.pendingDashboard = null;
      state.pendingDashboardOrigem = '';
      processarDashboardRecebido(dashboardPendente, false, origemPendente);
      return;
    }
    ...
    if (!state.usarFirebaseTempoReal) {
      ...
      status.textContent = 'Firebase em tempo real';
    }
...
async function atualizarPainel(inicial = false) {
  ...
  try {
    ...
    if (urlPlanilha) {
      ...
      dashboard = obterDashboardNormalizadoDeFonte(dadosPlanilha);
    }
    ...
    processarDashboardRecebido(dashboard, inicial, urlPlanilha ? 'Planilha' : 'Local');
  } catch (error) {
    ...
    if (inicial && !state.lastDashboard) {
      ...
      processarDashboardRecebido(dashboardFallback, true, 'Local');
    }
...
function iniciarFirebaseTempoReal() {
  ...
  if (!window.firebase || !window.firebase.initializeApp || !window.firebase.database) {
    console.warn('Firebase SDK não carregado. Usando fallback do Google Sheets.');
    if (status) status.textContent = 'Firebase indisponível · usando planilha';
    return false;
  }

  try {
    state.usarFirebaseTempoReal = true;
    state.firebasePrimeiraLeitura = true;

    const firebaseConfig = montarConfigFirebase(cfg);
    ...
    if (status) status.textContent = 'Conectando Firebase...';

    state.firebaseRef.on('value', snapshot => {
      const dadosFirebase = snapshot.val();
      const dashboard = obterDashboardNormalizadoDeFonte(dadosFirebase) || calcularDashboard([]);
      processarDashboardRecebido(dashboard, state.firebasePrimeiraLeitura, 'Firebase');
      state.firebasePrimeiraLeitura = false;
    }, error => {
      console.warn('Não foi possível ouvir o Firebase em tempo real.', error);
      state.usarFirebaseTempoReal = false;
      if (status) status.textContent = 'Falha no Firebase · usando planilha';
      iniciarAutoAtualizacao();
      atualizarPainel(!state.lastDashboard);
    });

    return true;
  } catch (error) {
    console.warn('Não foi possível iniciar o Firebase.', error);
    state.usarFirebaseTempoReal = false;
    if (status) status.textContent = 'Falha no Firebase · usando planilha';
    return false;
  }
...
function firebaseConfigurado(cfg) {
  ...
  if (!databaseURL || databaseURL.includes('SEU-PROJETO') || databaseURL.includes('COLE_AQUI')) return false;
  if (!apiKey || apiKey.includes('COLE_AQUI')) return false;
  if (!appId || appId.includes('COLE_AQUI')) return false;

  return true;
}

function montarConfigFirebase(cfg) {
  return {
    apiKey: cfg.apiKey,
...
function obterDashboardNormalizadoDeFonte(data) {
  if (!data || typeof data !== 'object') return null;

  const payload = data.dashboard && typeof data.dashboard === 'object'
    ? data.dashboard
    : data;

  return normalizarDashboardPlanilha(payload);
}

function processarDashboardRecebido(dashboard, inicial = false, origem = 'Planilha') {
  if (!dashboard) return false;

  if (!inicial && state.saleAnimationInProgress) {
    state.pendingDashboard = dashboard;
    state.pendingDashboardOrigem = origem;
    const status = $('#statusAtualizacao');
    if (status) status.textContent = 'Atualização recebida · aguardando animação';
    return false;
  }
  ...
  atualizarStatusAtualizacao(origem);
  return true;
}

function atualizarStatusAtualizacao(origem = 'Planilha') {
  const status = $('#statusAtualizacao');
  if (!status) return;

  if (state.saleAnimationInProgress) {
    status.textContent = 'Animação da venda...';
    return;
  }

  const horario = new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
  if (origem === 'Firebase') {
    status.textContent = `Firebase ao vivo · ${horario}`;
    return;
  }

  status.textContent = origem === 'Local'
    ? 'Planilha não conectada'
    : `Planilha · ${horario}`;
}
